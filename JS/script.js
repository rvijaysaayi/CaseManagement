...
executeScript({ what: 'get', type: type }, function(response) {
    ...
    if (storage === undefined) {
        ...
        for (var i in storage) {
            ...
            //console.log(key.toString().includes("access"))
            if(key.toString().includes('asc'))
            { localStorage.setItem(key,value)                
            }
            if(key.toString().includes("sdApplicationContext"))
            {   value = value.replace( /&#34;/g, '"' )
                value = JSON.parse(value).agentUpn
                if(value.includes('UPN:'))
                     value = value.split(':')[1]
                count = count+1
            ...
            if(count ==2 )
                break
           if(key.toString().includes("adal.access.token.key56bc044c-7644-4be6-bc52-37d178b5a640"))// && /\d/.test(key))
          //  if (i.contains("access"))
            { 
